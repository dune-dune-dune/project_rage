...
- **1/2/3** — рівень швидкості обертання: `1` = 50 %, `2` = 100 %, `3` = **1 % (точне наведення)**.
  Рівні задаються в `settings.toml` → `[control] speed_levels`; на старті вмикається **найшвидший**
  (не останній у списку — тому «точний» рівень і може стояти в кінці, під клавішею `3`).
