...
- **1/2** — рівень швидкості обертання (повільно/швидко; рівні задаються в `settings.toml`
  → `[control] speed_levels`, за замовчуванням найшвидший). Авто-трек не залежить від рівня.
...
- **⚙** (вгорі праворуч) — налаштування прицілу (зсув H/V, у `services/web/data/crosshair.json`) та
  ШІ: поріг впевненості (conf, дефолт 70%), мінімальний розмір об'єкта в пікселях і чутливість руху для
  CUSTOM (у `services/web/data/ai_settings.json`).
...
deadman) і список камер — у `services/web/settings.toml`.
